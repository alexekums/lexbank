...
function LexTXPage() {
  ...
  const [pairs, setPairs] = useState<ForexPair[]>(initialForex.slice(0, 4));
...
function ForexPanel({ pairs, onPair, onTrade }: { pairs: ForexPair[]; onPair: (pair: ForexPair) => void; onTrade: (pair: ForexPair, side: "BUY" | "SELL") => void }) {
  return <section><h2 className="mb-3 text-base font-black">Forex Terminal</h2><div className="overflow-hidden rounded-2xl bg-white/[0.04] ring-1 ring-white/10">{pairs.map((p, i) => <button key={p.pair} onClick={() => onPair(p)} className={`w-full px-4 py-3 text-left ${i ? "border-t border-white/5" : ""}`}><div className="flex items-center gap-3"><div className="flex-1"><p className="text-sm font-black">{p.pair}</p><p className={`text-[11px] font-bold ${p.change >= 0 ? "text-emerald-300" : "text-rose-300"}`}>{p.change >= 0 ? <TrendingUp className="inline h-3 w-3" /> : <TrendingDown className="inline h-3 w-3" />} {p.change >= 0 ? "+" : ""}{p.change.toFixed(2)}%</p></div><div className="text-right"><p className="font-mono text-xs text-white/70">{p.bid}</p><p className="font-mono text-xs text-white/40">{p.ask}</p></div><div className="flex gap-1"><span onClick={(e) => { e.stopPropagation(); onTrade(p, "BUY"); }} className="rounded-md bg-emerald-500 px-2 py-1 text-[10px] font-black shadow-[0_0_12px_rgba(16,185,129,0.65)]">BUY</span><span onClick={(e) => { e.stopPropagation(); onTrade(p, "SELL"); }} className="rounded-md bg-rose-500 px-2 py-1 text-[10px] font-black shadow-[0_0_12px_rgba(244,63,94,0.65)]">SELL</span></div></div></button>)}</div></section>;
}
...
function TradingScreen({ pair, onBack, onTrade }: { pair: ForexPair; onBack: () => void; onTrade: (side: "BUY" | "SELL") => void }) {
  ...
  return <div className="mx-auto min-h-screen max-w-md bg-stone-950 text-white"><header className="flex items-center gap-3 px-4 pb-3 pt-10"><button onClick={onBack} className="flex h-10 w-10 items-center justify-center rounded-full bg-white/10"><ArrowLeft className="h-5 w-5" /></button><div className="flex-1"><h1 className="text-lg font-black">{pair.pair}</h1><p className="text-xs text-white/50">Pip value · {(pair.pair.includes("JPY") ? 0.01 : 0.0001).toFixed(pair.pair.includes("JPY") ? 2 : 4)}</p></div><div className="text-right"><p className="font-mono text-sm font-black text-emerald-300">{mid.toFixed(decimals)}</p><p className="text-[10px] text-white/45">Spread {(pair.ask - pair.bid).toFixed(decimals)}</p></div></header><section className="px-4"><div className="mb-2 flex gap-2 overflow-x-auto pb-1"><ToolPill icon={MoveDiagonal} label="Trendline" /><ToolPill icon={DraftingCompass} label="Fib" /><ToolPill icon={Square} label="Shapes" /><ToolPill icon={Circle} label="Zones" /><ToolPill icon={Layers} label="Layers" /></div><div className="relative h-80 overflow-hidden rounded-2xl bg-gradient-to-b from-red-950/40 to-black ring-1 ring-white/10"><div className="absolute inset-0 bg-[linear-gradient(rgba(255,255,255,0.055)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.055)_1px,transparent_1px)] bg-[size:36px_30px]" /><svg viewBox="0 0 360 280" className="absolute inset-0 h-full w-full"><g opacity="0.45" stroke="rgb(255 255 255)"><line x1="0" y1="56" x2="360" y2="56" strokeDasharray="4 8" /><line x1="0" y1="116" x2="360" y2="116" strokeDasharray="4 8" /><line x1="0" y1="176" x2="360" y2="176" strokeDasharray="4 8" /></g><g>{[26,54,82,110,138,166,194,222,250,278,306,334].map((x, i) => <rect key={x} x={x} y={[166,146,154,112,126,82,98,64,78,42,58,36][i]} width="10" height={[42,34,46,54,32,48,38,44,36,52,34,40][i]} rx="2" fill={i % 3 === 0 ? "rgb(244 63 94)" : "rgb(52 211 153)"} />)}</g><polyline points="0,210 32,188 68,196 104,146 140,158 176,105 212,122 248,82 284,96 326,52 360,66" fill="none" stroke="rgb(52 211 153)" strokeWidth="2.5" /><line x1="42" y1="218" x2="318" y2="62" stroke="rgb(255 75 75)" strokeWidth="2" strokeDasharray="7 7" /><g stroke="rgb(251 191 36)" strokeWidth="1.5" opacity="0.85"><line x1="62" y1="76" x2="306" y2="76" /><line x1="62" y1="124" x2="306" y2="124" /><line x1="62" y1="172" x2="306" y2="172" /></g><rect x="74" y="86" width="70" height="52" rx="6" fill="none" stroke="rgb(248 113 113)" strokeDasharray="5 5" /></svg><div className="absolute right-2 top-4 space-y-8 text-right font-mono text-[10px] text-white/55"><p>{(mid * 1.002).toFixed(decimals)}</p><p>{mid.toFixed(decimals)}</p><p>{(mid * 0.998).toFixed(decimals)}</p></div><div className="absolute bottom-3 left-3 flex flex-wrap gap-2"><span className="rounded-full bg-white/10 px-2 py-1 text-[10px] font-bold"><MoveDiagonal className="mr-1 inline h-3 w-3" />Trendline</span><span className="rounded-full bg-white/10 px-2 py-1 text-[10px] font-bold"><DraftingCompass className="mr-1 inline h-3 w-3" />Fib 38.2 / 61.8</span><span className="rounded-full bg-white/10 px-2 py-1 text-[10px] font-bold">Pips + price scale</span></div></div><div className="mt-3 grid grid-cols-2 gap-2"><button onClick={() => onTrade("BUY")} className="h-12 rounded-xl bg-emerald-500 text-sm font-black shadow-[0_0_20px_rgba(16,185,129,0.6)]">BUY</button><button onClick={() => onTrade("SELL")} className="h-12 rounded-xl bg-rose-500 text-sm font-black shadow-[0_0_20px_rgba(244,63,94,0.6)]">SELL</button></div></section><section className="px-4 py-5"><h2 className="mb-3 flex items-center gap-2 text-sm font-black"><MessageCircle className="h-4 w-4" />Chart chat & analysis</h2><form onSubmit={submit} className="flex gap-2"><input value={note} onChange={(e) => setNote(e.target.value)} placeholder="Type a comment or trade idea…" className="min-w-0 flex-1 rounded-xl bg-white/10 px-3 py-3 text-sm outline-none ring-1 ring-white/10 focus:ring-primary" /><button className="rounded-xl bg-gradient-primary px-4 text-sm font-black">Post</button></form><ul className="mt-3 space-y-2">{notes.map((n, i) => <li key={`${n}-${i}`} className="rounded-xl bg-white/[0.05] p-3 text-xs text-white/75 ring-1 ring-white/10">{n}</li>)}</ul></section></div>;
}
